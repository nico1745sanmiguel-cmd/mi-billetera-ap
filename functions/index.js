...
// ─── DIAGNÓSTICO: INSPECCIONAR Y ENVIAR PUSH DIRECTO ───
// Callable desde el frontend. Devuelve el estado real del usuario en Firestore
// y opcionalmente envía un push directo al token guardado.
exports.diagnosePush = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Debe estar autenticado.');
    }

    const uid = context.auth.uid;
    const result = { uid, steps: [] };

    try {
        // Paso 1: leer el documento del usuario
        const userDoc = await admin.firestore().collection('users').doc(uid).get();
        if (!userDoc.exists) {
            result.steps.push({ step: 'user_doc', ok: false, msg: 'El documento de usuario NO existe en Firestore.' });
            return result;
        }
        ...
        result.steps.push({
            ...
            fcmTokensCount: (userData.fcmTokens || []).length,
            fcmTokensPresent: !!userData.fcmTokens,
        });

        // Paso 2: verificar householdId
        const householdId = userData.householdId;
        if (!householdId) {
            result.steps.push({ step: 'household', ok: false, msg: 'El usuario NO tiene householdId guardado. La Cloud Function no puede encontrarlo.' });
            return result;
        }
        result.steps.push({ step: 'household', ok: true, householdId });

        // Paso 3: verificar tokens
        const tokens = userData.fcmTokens || [];
        if (tokens.length === 0) {
            result.steps.push({ step: 'tokens', ok: false, msg: 'No hay tokens FCM guardados. El usuario nunca completó "Activar Notificaciones" o falló al guardar.' });
            return result;
        }
        ...
        result.steps.push({ step: 'error', ok: false, msg: error.message });
        return result;
    }
});
