                        ...
                        {isMe ? (
                            editingUid === member.id ? (
                                <div className="flex gap-2">
                                    ...
                                    />
                                    <button onClick={() => handleSaveSalary(member.id)} disabled={saving} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-xl disabled:opacity-50 transition-colors">
                                        {saving ? '...' : 'Guardar'}
                                    </button>
                                    <button onClick={() => setEditingUid(null)} className={`px-3 py-2 rounded-xl text-sm font-bold transition-colors ${isGlass ? 'bg-white/10 text-gray-300' : 'bg-gray-100 text-gray-500'}`}>✕</button>
                                </div>
                            ) : (
                                <button onClick={() => handleStartEdit(member)} className={`w-full flex items-center justify-between p-3 rounded-xl border-dashed border-2 transition-colors group ${salary > 0 ? (isGlass ? 'border-indigo-500/40 hover:border-indigo-400' : 'border-indigo-200 hover:border-indigo-400') : (isGlass ? 'border-white/20 hover:border-white/40' : 'border-gray-200 hover:border-gray-400')}`}>
