...
// AHORA RECIBE 'currentDate' DESDE APP.JSX
export default function ServicesManager({ services = [], cards = [], transactions = [], currentDate }) {
  ...
  // USAMOS LA FECHA QUE VIENE DE ARRIBA
  ...
  // ACCIONES (Sin cambios de lógica)
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-24">
      ...
      {/* 1. GRÁFICO DE CARGA SEMANAL (Sin el navegador de meses viejo) */}
          ...
          <div className="flex items-end justify-between h-24 gap-2">
              {weeklyData.map((week, idx) => (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-1 group">
                      ...
                      <div className="w-full bg-blue-500 rounded-t-md transition-all duration-500 hover:bg-blue-400 relative" style={{ height: `${week.height || 2}%`, opacity: week.value > 0 ? 1 : 0.2 }}></div>
      ...
      <div className="space-y-3">
          <div className="flex justify-between items-center px-2">
              ...
              <button onClick={() => openModal()} className="text-xs bg-gray-900 text-white px-3 py-1.5 rounded-lg font-bold shadow-md hover:bg-black">+ Nuevo</button>
          </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4 animate-fade-in">
            <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl animate-slide-up">
                <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-bold text-gray-800">{editingService ? 'Editar Servicio' : 'Nuevo Servicio'}</h3><button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600">✕</button></div>
                <form onSubmit={handleSave} className="space-y-4">
                    <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1">Nombre</label><input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold text-gray-800" placeholder="Ej: Luz" value={form.name} onChange={e => setForm({...form, name: e.target.value})} /></div>
                    <div className="grid grid-cols-2 gap-3">
                        <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1">Monto ($)</label><input type="tel" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold text-gray-800 text-center" placeholder="0" value={new Intl.NumberFormat('es-AR').format(form.amount)} onChange={handleAmountChange} /></div>
                        <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1">Día Venc.</label><input type="number" max="31" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold text-gray-800 text-center" placeholder="10" value={form.day} onChange={e => setForm({...form, day: e.target.value})} /></div>
                    </div>
                    <div>
                    ...
                    <div className="pt-4 flex gap-3">
                        {editingService && <button type="button" onClick={handleDelete} className="p-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-100"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>}
                        <button type="submit" className="flex-1 bg-gray-900 text-white font-bold rounded-xl py-3 shadow-lg hover:bg-black">{editingService ? 'Actualizar Monto' : 'Guardar'}</button>
                    </div>
                </form>
            </div>
        </div>
