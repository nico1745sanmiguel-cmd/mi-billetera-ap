...
export default function ServicesManager({ services = [], cards = [], transactions = [], currentDate }) {
  ...
  // 2. TARJETAS (LOGICA CORREGIDA: VENCIMIENTO Y AJUSTE MANUAL)
  const cardServices = useMemo(() => {
      const targetMonthVal = currentDate.getFullYear() * 12 + currentDate.getMonth();

      return cards.map(c => {
          // A. ¿Existe un ajuste manual (Resumen Oficial) para este mes?
          const manualAmount = c.adjustments?.[currentMonthKey];

          // B. Si no, calculamos la suma de cuotas
          let debt = 0;
          if (manualAmount !== undefined) {
              debt = manualAmount; // <--- USAMOS EL DATO REAL DEL RESUMEN
          } else {
              debt = transactions
          ...
          return { 
              ...
              name: c.name, 
              amount: debt, 
              day: c.dueDay || 10, // <--- CORRECCIÓN: USAMOS DUE DAY (VENCIMIENTO)
              isPaid, 
              type: 'card', 
              bank: c.bank, 
              frequency: 'Mensual',
              isManual: manualAmount !== undefined // Para mostrar un ícono si está editado
          };
      }).filter(Boolean);
  ...
  const handleSave = async (e) => {
      ...
      try {
          // LÓGICA ESPECIAL PARA TARJETAS (Ajuste de Resumen)
          if (editingService && editingService.type === 'card') {
              const cardRef = doc(db, 'cards', editingService.id);
              await setDoc(cardRef, {
                  adjustments: {
                      [currentMonthKey]: Number(form.amount) // Guardamos el monto manual para ESTE mes
                  }
              }, { merge: true });
  ...
  const handleDelete = async () => { 
      if (editingService.type === 'card') {
          if(window.confirm("¿Quitar ajuste manual y volver al cálculo automático?")) {
             // Borramos el ajuste manual
              ...
              <div key={item.id} className={`bg-white p-4 rounded-xl border transition-all duration-300 group ${item.isPaid ? 'border-green-200 bg-green-50/30 opacity-75' : 'border-gray-100 hover:border-blue-300 shadow-sm hover:shadow-md'}`}>
                  <div className="flex justify-between items-center">
                      <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center text-xs font-bold transition-colors border ${item.isPaid ? 'bg-green-100 text-green-700 border-green-200' : 'bg-gray-50 text-gray-600 border-gray-100'}`}><span className="text-lg leading-none">{item.day}</span><span className="text-[8px] uppercase opacity-70">Vence</span></div>
                          <div>
                              <div className="flex items-center gap-2">
                                  ...
                                  {item.type === 'card' && <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold border ${item.isManual ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-blue-100 text-blue-700 border-blue-200'}`}>{item.bank} {item.isManual && '✎'}</span>}
                ...
                <form onSubmit={handleSave} className="space-y-5 pb-6">
                    {editingService?.type === 'card' ? (
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                            <p className="text-xs text-blue-600 mb-2 font-medium">Ingresa el monto exacto de tu resumen final. Esto reemplazará la suma automática para este mes.</p>
