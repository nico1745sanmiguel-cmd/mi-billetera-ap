            ...
            <div className="flex items-end justify-between h-40 gap-4 px-2 relative z-10">
                {weeklyData.map((week, idx) => (
                    <div key={week.label || idx} className="flex-1 flex flex-col items-center gap-3 group relative h-full justify-end">
                        ...
                        <div className="w-full h-full bg-gray-800/50 rounded-t-lg relative flex items-end overflow-hidden origin-bottom transition-transform duration-500 will-change-transform" style={{ transform: `scaleY(${Math.min(Math.max(week.heightTotal || 0, 0), 100) / 100})` }}>
                            <div className={`w-full h-full origin-bottom transition-transform duration-1000 ease-out relative will-change-transform ${week.percentFilled >= 100 ? 'bg-green-400 shadow-[0_0_20px_rgba(74,222,128,0.4)]' : 'bg-green-500/80'}`} style={{ transform: `scaleY(${Math.min(Math.max(week.percentFilled || 0, 0), 100) / 100})` }}>
