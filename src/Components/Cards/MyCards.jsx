...
export default function MyCards({ cards, privacyMode, currentDate }) {
  ...
  const initialForm = { name: '', bank: '', closeDay: '', dueDay: '', color: PRESET_COLORS[0] };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button type="button" onClick={() => openForm()} className="bg-brand-primary text-surface-light px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 shadow-sm transition-colors">+ Agregar Tarjeta</button>
      </div>
                ...
                <div className="pt-2">
                  {stmt ? (
                    <div className="grid grid-cols-2 gap-2 items-end">
                      <div>
                        <p className="text-[9px] opacity-80 uppercase tracking-widest drop-shadow-sm">Total a pagar</p>
                        <p className="font-mono text-lg tracking-wide font-bold text-shadow-sm">{showMoney(stmt.totalDue)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[9px] opacity-80 uppercase drop-shadow-sm">Vence</p>
                        <p className="font-mono text-sm font-bold opacity-90">
                          {stmt.dueDate ? new Date(stmt.dueDate + 'T12:00:00').toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit' }) : '--'}
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 opacity-60">
                      ...
                      <p className="text-xs">Sin resumen este mes</p>
                    </div>
                  )}
                </div>
              </div>
              <div className="absolute top-3 end-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-20">
                <button type="button" aria-label="Editar" onClick={() => openForm(card)} className="bg-white/20 hover:bg-white/40 backdrop-blur-md text-white p-2 rounded-full shadow-lg" title="Editar">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                </button>
                <button type="button" aria-label="Borrar" onClick={() => handleDelete(card.id)} className="bg-status-danger hover:bg-red-600 backdrop-blur-md text-white p-2 rounded-full shadow-lg" title="Borrar">
