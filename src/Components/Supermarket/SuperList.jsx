    ...
    return (
        <div className="animate-fade-in pb-32">
            ...
            <div className="sticky top-0 z-30 bg-[#f3f4f6]/95 backdrop-blur-sm pt-2 pb-3 mb-2 transition-all shadow-sm -mx-4 px-6 border-b border-gray-200/50">
                ...
                <div className={`absolute top-0 left-0 right-0 flex justify-center transition-all duration-300 ${toast ? 'translate-y-4 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`}>
                    <div className="bg-gray-900/90 backdrop-blur text-white px-4 py-2 rounded-full shadow-xl flex items-center gap-3 text-xs font-bold z-50">
                        <span>{toast?.message}</span>
                        {toast?.undoAction && (
                            <button onClick={handleUndo} className="text-yellow-400 hover:text-yellow-300 uppercase tracking-wider">
                                Deshacer
                            </button>
                        )}
                    </div>
