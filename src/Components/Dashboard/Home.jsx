...
export default function Home({ transactions, cards, supermarketItems = [], services = [], privacyMode, setView, onLogout, currentDate, user }) {
  ...
  // B. Supermercado (Lógica Estimado vs Realizado)
  const superData = useMemo(() => {
      ...
      // Si gasté algo (>0), muestro lo "Realizado". Si no (0), muestro el "Estimado".
      const showAmount = realSpent > 0 ? realSpent : totalBudget;
      const label = realSpent > 0 ? 'Realizado' : 'Pendiente';
      const statusColor = realSpent > 0 ? 'text-gray-800' : 'text-gray-400';
  ...
  const WIDGETS = {
    ...
    // TARJETAS REIMAGINADAS (Con Logos)
    cards: (
      <div>
                  ...
                  {cards.map((card) => {
                      ...
                      return (
                          <div key={card.id} className="flex-shrink-0 w-[85%] max-w-[280px] h-44 rounded-2xl shadow-lg p-5 text-white relative overflow-hidden snap-center transition-transform active:scale-95 group" style={{ background: `linear-gradient(135deg, ${card.color || '#1f2937'} 0%, ${card.color || '#111827'}DD 100%)` }} onClick={() => setView('cards')}>
                              ...
                              {/* Footer Tarjeta: Vencimiento y Números sin asteriscos */}
    ...
    super_actions: (
      <div className="grid grid-cols-2 gap-3 mx-1">
          {/* BOTÓN SUPERMERCADO INTELIGENTE */}
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      {/* BOTÓN DINÁMICO DOBLE ALTO (ANÁLISIS) */}
      <button 
