...
export default function Home({ transactions, cards, supermarketItems = [], services = [], privacyMode, setView, onLogout }) {
  ...
  const agenda = useMemo(() => {
      const realServices = services.map(s => ({ id: s.id, name: s.name, amount: s.amount, day: s.day, isPaid: s.isPaid || false, type: 'service' }));
  ...
  const criticalAlert = useMemo(() => {
      ...
      if (urgentItem) return { active: true, msg: `¡Atención! ${urgentItem.name} vence ${urgentItem.day === today ? 'HOY' : 'mañana'}.`, amount: urgentItem.amount };
      return { active: false };
  }, [agenda]);
      ...
      <div>
          <div className="flex justify-between items-center px-2 mb-3">
              <h3 className="font-bold text-gray-800 text-sm flex items-center gap-2">
                 💳 Tus Tarjetas
                 <span className="text-[9px] bg-gray-100 px-1.5 rounded text-gray-400 font-normal">Desliza</span>
              </h3>
              ...
              <div className="flex overflow-x-auto gap-4 pb-4 px-2 snap-x snap-mandatory hide-scrollbar">
                  {cards.map((card) => (
                      <div key={card.id} className="flex-shrink-0 w-[85%] max-w-[280px] h-40 rounded-2xl shadow-md p-5 text-white relative overflow-hidden snap-center transition-transform active:scale-95" style={{ background: `linear-gradient(135deg, ${card.color || '#333'} 0%, ${card.color || '#333'}EE 100%)` }} onClick={() => setView('cards')}>
                          ...
                          <div><p className="text-[10px] opacity-70 uppercase mb-1">A pagar este mes</p><p className="font-mono text-2xl font-bold tracking-tight">{showMoney(cardsWithDebt.find(c=>c.id === card.id)?.currentDebt || 0)}</p></div>
    ...
    agenda: (
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mx-1">
          ...
          <div>
              {agenda.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                      <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center text-[10px] font-bold ${item.day <= new Date().getDate() + 3 ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-500'}`}><span className="text-sm">{item.day}</span><span className="text-[8px] uppercase">Vence</span></div>
              ...
              {agenda.length === 0 && <div className="p-6 text-center text-gray-400"><p className="text-xs">🎉 Todo al día por hoy</p></div>}
          </div>
      </div>
    ),
    ...
    super_actions: (
      <div className="grid grid-cols-2 gap-3 mx-1">
          <div onClick={() => setView('super')} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm cursor-pointer hover:border-purple-200 transition-colors group">
              ...
              <p className="text-xs text-gray-400 font-bold uppercase">Supermercado</p><p className="text-lg font-bold text-gray-800">{showMoney(superData.realSpent)}</p>
          </div>
          <div onClick={() => setView('purchase')} className="bg-gray-900 p-4 rounded-2xl shadow-lg cursor-pointer active:scale-95 transition-all flex flex-col justify-between group">
              ...
              <div><p className="text-xs text-gray-400 font-bold uppercase">Registrar</p><p className="text-lg font-bold text-white">Nuevo Gasto</p></div>
          </div>
      </div>
    )
  };
