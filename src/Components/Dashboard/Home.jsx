...
export default function Home({ transactions, cards, supermarketItems = [], services = [], privacyMode, setView, onLogout, currentDate, user }) {
  ...
  // B. Supermercado (CORREGIDO: Ahora incluye items viejos sin fecha) 🛒
  const superData = useMemo(() => {
      // FILTRO ROBUSTO (Igual que en SuperList)
      const monthlyItems = supermarketItems.filter(item => {
          if (item.month) return item.month === targetMonthKey;
          
          // Compatibilidad: Si no tiene mes, asumimos que es del mes actual real
